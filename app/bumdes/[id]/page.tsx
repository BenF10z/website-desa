...
interface BumdesItem {
  ...
  location: string;
  image_url: string;
  additional_images?: string[]; // Added this field
  is_active: boolean;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="w-full px-4 md:px-[120px] pt-[60px] pb-[50px] flex justify-center items-start">
        <div className="w-full max-w-7xl flex flex-col lg:flex-row justify-start items-start gap-8 lg:gap-20">
          ...
          <div className="flex-1 max-w-[648px] flex flex-col justify-start items-start gap-6 order-1 lg:order-2">
            {(() => {
              const descriptionParts = umkmDetail.description.split('\n\n');
              const additionalImages = umkmDetail.additional_images || [];
              const content = [];
              
              // Calculate how many paragraphs per image
              const insertImageAfter = Math.ceil(descriptionParts.length / (additionalImages.length + 1));
              ...
              descriptionParts.forEach((part, index) => {
                // Add paragraph
                content.push(
                  <div key={`description-${index}`} className="w-full flex flex-col justify-start items-start">
                ...
                // After every few paragraphs, insert image
                    ...
                    <div key={`image-${imageIndex}`} className="w-full flex flex-col justify-center items-center gap-4 my-4">
                      <div className="relative w-full h-[300px] md:h-[400px] rounded-lg overflow-hidden shadow-lg">
                        <Image
                          ...
                          alt={`${umkmDetail.name} - Gambar ${imageIndex + 1}`}
                          fill
                          className="object-cover"
                        />
                      </div>
                ...
                content.push(
                  <div key={`remaining-image-${imageIndex}`} className="w-full flex flex-col justify-center items-center gap-4 my-4">
                    ...
                    <p className="text-[#6e7869] text-sm italic text-center">
                      Gambar {imageIndex + 1}: Galeri {umkmDetail.name}
                    </p>
                  </div>
                );
                imageIndex++;
              ...
              content.push(
                <div key="contact-info" className="w-full flex flex-col justify-start items-start">
                  <div className="w-full flex justify-start items-start gap-2.5">
                    <div className="flex-1 space-y-6">
                      <div>
